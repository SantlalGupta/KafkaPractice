 ...
 *
 * This class demonstrate sending messages in batch with compression
 */
public class MyTwitterProducerWithHighThroughputUsingCompression {
    private Logger logger = LoggerFactory.getLogger(MyTwitterProducerWithHighThroughputUsingCompression.class.getName());

    // list of word for which we want to get twittes
    List<String> terms = Lists.newArrayList("kafka","IOT","java","hadoop","Spark","technology");

    public static void main(String[] args) {
        new MyTwitterProducerWithHighThroughputUsingCompression().run();
    }
    ...
    private KafkaProducer<String,String> createKafkaProducer() {
        ...
        properties.setProperty(ProducerConfig.COMPRESSION_TYPE_CONFIG,map.get(KafkaProperty.COMPRESSION_TYPE));
        properties.setProperty(ProducerConfig.LINGER_MS_CONFIG,map.get(KafkaProperty.LINGER_MS));
        properties.setProperty(ProducerConfig.BATCH_SIZE_CONFIG,
                Integer.toString(Integer.parseInt(map.get(KafkaProperty.BATCH_SIZE))*1024));
